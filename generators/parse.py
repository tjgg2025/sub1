...
def parse_code_block(string: str, lang: str) -> Optional[str]:
    # Try pattern 1: ```python\n...\n```
    code_pattern = fr"```{lang}\n(.*?)\n```"
    match = re.search(code_pattern, string, re.DOTALL)

    if match:
        print(f"DEBUG: parse_code_block - matched language-specific pattern ```{lang}")
    ...
    if match:
        print(f"DEBUG: parse_code_block - matched generic code block pattern ```")
        return match.group(1)

    # Fall back to parse_first_func
    print(f"DEBUG: parse_code_block - no code blocks found, trying parse_first_func")
    result = parse_first_func(string, lang)
    if result is None:
        print(f"DEBUG: parse_code_block - parse_first_func also returned None (no 'def ' found)")
    return result
